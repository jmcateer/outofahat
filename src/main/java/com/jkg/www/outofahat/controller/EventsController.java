...
public class EventsController {
    ...
    @ResponseBody
    public ResponseEntity<? extends IResponseMessage> createEvent(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 500, message = "Error", response = ServiceResponse.class)})
    @ResponseBody
    public ResponseEntity<? extends IResponseMessage> saveEvent(
            @PathVariable String userId,
    ...
    @RequestMapping(path = "/v1/event/{userId}/create/save", produces = "application/json", method = RequestMethod.POST)
    @ApiOperation(value = "create an event for user", nickname = "create an event for user")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Success", response = ServiceResponse.class),
            @ApiResponse(code = 500, message = "Error", response = ServiceResponse.class)})
    @ResponseBody
    public ResponseEntity<? extends IResponseMessage> createAndSaveEvent(
            @PathVariable String userId,
            @RequestBody NewEventRequest eventRequest) {
        ServiceResponse<String> response = eventService.createAndSaveEvent(userId, eventRequest);
        return responseEntityMapper.map(response);
    }
    ...
    @RequestMapping(path = "/v1/event/{userId}/events/{event}/invite", produces = "application/json", method = RequestMethod.GET)
    @ApiOperation(value = "get events for user", nickname = "get events for user")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Success", response = ServiceResponse.class),
            @ApiResponse(code = 500, message = "Error", response = ServiceResponse.class)})
    @ResponseBody
    public ResponseEntity<? extends IResponseMessage> sendInvites(
            @PathVariable String userId,
            @PathVariable String event) {
        ServiceResponse<List<String>> response = eventService.sendInvites(userId, event);
        return responseEntityMapper.map(response);
    }
}
