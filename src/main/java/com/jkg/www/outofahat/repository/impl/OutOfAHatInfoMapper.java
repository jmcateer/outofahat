...
public class OutOfAHatInfoMapper {
    ...
    /**
     * Create a new participantId from the name.  If the participantId already exists in the list of ParticipantDbo, a
     * random value will be added to the result to make it unique.
     *
     ...
     */
    public String createParticipantId(
            String first,
            String last,
        ...
        if (participants != null) {
            Optional<ParticipantDbo> optional = participants.stream()
                    .filter(participant -> participant.getParticipantId().equals(id))
                    .findFirst();
    ...
    /**
     * Create a ParticipantDbo from an NewParticipantRequest.  The participantId for the new object will be generated
     * checking the uniqueness from the list of ParticipantDbo.
     *
     * @param participantRequest New participant request.
     * @param participants List of participants to check new id against for uniqueness.
     * @return new ParticipantDbo object.
     */
    public ParticipantDbo mapFromNewParticipantRequest(
            NewParticipantRequest participantRequest,
            List<ParticipantDbo> participants) {
        ...
        return new ParticipantDbo(
                ...
                contactInfoDbo,
                true,
                participantRequest.getIneligibles(),
    ...
    public Participant mapToParticipant(final ParticipantDbo participantDbo) {
        return new Participant(
                participantDbo.getParticipantId(),
